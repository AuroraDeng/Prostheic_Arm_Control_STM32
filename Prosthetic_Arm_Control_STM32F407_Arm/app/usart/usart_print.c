...
//用串口2给BWT模块发送指令
void SendCmdtoBWT(const char cmd[])
{
	print_str(&UART2_Handler,(char*)cmd);
}

void SendCmdtoKWR(const char cmd[])
{
	print_str(&UART3_Handler,(char*)cmd);
}
...
void SendCmdtoHWT(uint8_t *p_data,unsigned int uiSize)
{
	while(__HAL_UART_GET_FLAG(&UART4_Handler, UART_FLAG_TXE) == RESET){}
		HAL_UART_Transmit(&UART4_Handler, (uint8_t*)p_data,uiSize,100); // 发送数据		
	while(__HAL_UART_GET_FLAG(&UART4_Handler, UART_FLAG_TXE) == RESET);
}
