...
void AutoScanSensor(void)
{
		...
		do
		{
			...
			if(s_cDataUpdate != 0)
			{
				printf("find sensor successfully\r\n\r\n");
				ShowHelp();
				return ;
			}
			iRetry--;
...
void SensorDataUpdata(uint32_t uiReg, uint32_t uiRegNum)//更新传感器数据
{
		int i;
...
//		CmdProcess();
		if(s_cDataUpdate)
		{
			...
			for(i = 0; i < 3; i++)
			{
				ElbowKdata.fAngle[i] = sReg[Roll+i] / 32768.0f * 180.0f;
				ElbowKdata.fGyro[i] = sReg[GX+i] / 32768.0f * 2000.0f;
				uint32_t runtime=stop_timer();
				if(runtime!=0)
					ElbowKdata.fAcc[i] = (ElbowKdata.fGyro[i]-pre_v[i])/(runtime/1000);
				start_timer();
				pre_v[i]=ElbowKdata.fGyro[i];
			}
				
//			if(s_cDataUpdate & GYRO_UPDATE)
//			{
				printf("gyro:%.3f %.3f %.3f\r\n", ElbowKdata.fGyro[0], ElbowKdata.fGyro[1], ElbowKdata.fGyro[2]);
				printf("acc:%.3f %.3f %.3f\r\n", ElbowKdata.fAcc[0], ElbowKdata.fAcc[1], ElbowKdata.fAcc[2]);
//				s_cDataUpdate &= ~GYRO_UPDATE;
//			}
//			if(s_cDataUpdate & ANGLE_UPDATE)
//			{
				printf("angle:%.3f %.3f %.3f\r\n", ElbowKdata.fAngle[0], ElbowKdata.fAngle[1], ElbowKdata.fAngle[2]);
//				s_cDataUpdate &= ~ANGLE_UPDATE;
//			}
		}
}
	...
	switch(s_cCmd)
	{
		...
		case 'B':	
			if(WitSetUartBaud(WIT_BAUD_115200) != WIT_HAL_OK) 
				...
				uart4_init(c_uiBaud[WIT_BAUD_115200]);											
			break;
		case 'b':	
			if(WitSetUartBaud(WIT_BAUD_9600) != WIT_HAL_OK)
				printf("\r\nSet Baud Error\r\n");
			else 
				uart4_init(c_uiBaud[WIT_BAUD_9600]);												
			break;
		case 'R':	
...
void WitSerialDataIn(uint8_t ucData)//接收串口传入数据
{
    ...
    switch(s_uiProtoclo)
    {
        case WIT_PROTOCOL_NORMAL:
            if(s_ucWitDataBuff[0] != 0x55)//判断数据是否是 0x55 开始的
            {
                s_uiWitDataCnt--;
                memcpy(s_ucWitDataBuff, &s_ucWitDataBuff[1], s_uiWitDataCnt);//将数据 buff 中的第一个字节删除，并将计数器减一
                return;
            }
...
int32_t WitReadReg(uint32_t uiReg, uint32_t uiReadNum)//读寄存器指令
{
    uint16_t usTemp, i;
    uint8_t ucBuff[8];
    ...
    switch(s_uiProtoclo)
    {
        case WIT_PROTOCOL_NORMAL:
            ...
            ucBuff[3] = uiReg & 0xff;
            ucBuff[4] = uiReg >> 8;
						p_WitSerialWriteFunc(ucBuff,5);//通过串口发送指令
            break;
//        case WIT_PROTOCOL_MODBUS:
//            if(p_WitSerialWriteFunc == NULL)return WIT_HAL_EMPTY;
//            usTemp = uiReadNum << 1;
