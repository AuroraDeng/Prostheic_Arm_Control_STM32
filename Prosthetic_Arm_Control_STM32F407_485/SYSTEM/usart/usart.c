...
//	if(__HAL_UART_GET_IT_SOURCE(&UART4_Handler, UART_IT_RXNE) != RESET && __HAL_UART_GET_FLAG(&UART4_Handler, UART_FLAG_RXNE) != RESET)
//	{
//		HAL_UART_Receive_IT(&UART4_Handler,dRxBuffer, RXBUFFERSIZE);//该函数会开启接收中断：标志位UART_IT_RXNE，并且设置接收缓冲以及接收缓冲接收最大数据
//		WitSerialDataIn(dRxBuffer[0]);
//		__HAL_UART_CLEAR_FLAG(&UART4_Handler,UART_FLAG_RXNE);//UART_FLAG_RXNE：表示UART接收数据寄存器非空，即接收缓冲区已经有数据可以被读取。
//		__HAL_UART_CLEAR_FLAG(&UART4_Handler,UART_IT_RXNE);//UART_IT_RXNE：表示UART接收中断，即当接收数据寄存器非空时触发的中断。
//		__HAL_UART_CLEAR_FLAG(&UART4_Handler,UART_FLAG_ORE);
//	}
} 
