...
//			int32_t SendCommand[3]={0};
			float SendCommand[4]={0};
			Get_USART_Command(&UART1_Handler,SendCommand);	
				
			for(i=0;i<3;i++)
				SendStatus[i]=xQueueSend(WristCommandQueue,&SendCommand[i],0);
			SendStatus[3]=xQueueSend(ElbowCommandQueue,&SendCommand[3],0);
			
			if(SendStatus[0]==pdPASS&&SendStatus[1]==pdPASS&&SendStatus[2]==pdPASS)
			{
				println_str(&UART1_Handler,"Send to WristCommandQueue successfully!");
				xEventGroupSetBits(EventGroupHandler,0x01);
			}
			else
				println_str(&UART1_Handler,"Could not send to WristCommandQueue!");
			
			if(SendStatus[3]==pdPASS)
			{
				println_str(&UART1_Handler,"Send to ElbowCommandQueue successfully!");
				xEventGroupSetBits(EventGroupHandler,0x02);
			}
			else
				println_str(&UART1_Handler,"Could not send to ElbowCommandQueue!");	
		}
		else
		{
			times++;
